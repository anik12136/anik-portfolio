  ...
  const onSubmit = async (data, e) => {
    ...
    try {
      const response = await fetch(
        "https://ul-earning-server.vercel.app/formCourses",
        {
          // const response = await fetch('https://ul-earning-server.vercel.app/formCourses', {
          method: "POST",
          headers: {
    ...
    <div className=" ">
      <div className="flex justify-center">
        <div className="border-2 flex justify-between items-center px-24 w-5/6 py-8">
          ...
          <Link to={"/addCourse"}>
            ...
            <button className="text-black bg-yellow-400 px-16 py-3 font-semibold hover:text-yellow-500 hover:bg-black ">
        ...
        <form className="w-2/4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="py-2">
            {featureFields.map((field, index) => (
              <div key={field.id}>
                <input
                  className="border-2 border-black mt-3 text-sm w-60 ps-3 py-2"
                  type="text"
            ...
            <button
              type="button"
              className="text-sm border-2 border-black  rounded-full  py-3 mt-2 b w-full "
              onClick={() => append()}
            >
              + Add Feature
            </button>
          ...
          <button
            className="p-4 mt-2 w-full text-white bg-black hover:text-black hover:bg-yellow-500 flex justify-center items-center gap-3 mb-"
            type="submit"
            value="Post"
          >
            Submit
      ...
      <div>
        <div className="grid justify-center my-8">
          <Link to={"/instructor"}>
            <button className="text-center border px-3 py-2 border-zinc-950  hover:bg-gray-400 hover:text-white">
