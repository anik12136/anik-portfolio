...
const CartItems = ({ cartItem }) => {
  ...
  return (
    <div className="flex justify-between border-t-2 px-4 bg-slate-100 mb-3">
      ...
      <div>
        <button onClick={() => handleDelete(cartItem._id)}
          className=" text-red-700 text-3xl hover:text-red-500 pt-4">
            <MdDeleteForever />
        </button>
        {/* <button onClick={() => handleDelete(cartItem._id)}
          className="border-2 rounded-md  border-[#7ea58c] dark:border-white dark:text-white w-auto h-auto px-6 py-3 font-bold hover:text-yellow-500 dark:hover:text-black hover:bg-[#7ea58c] dark:hover:bg-white ease-in duration-100">
            <p>buy</p>
        </button> */}
      </div>
    </div>
